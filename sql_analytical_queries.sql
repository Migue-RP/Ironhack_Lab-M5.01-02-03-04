...
INSERT INTO sales VALUES
 ...
 --Create CTE for revenue calculation:
...
SELECT
  ...
  CASE
    WHEN daily_revenue > LAG(daily_revenue, 1) OVER (ORDER BY order_date) THEN 'INCREASE'
    WHEN daily_revenue < LAG(daily_revenue, 1) OVER (ORDER BY order_date) THEN 'DECREASE'
    ELSE 'EQUAL'
  END AS trend
FROM daily_revenue
ORDER BY order_date;
...
SELECT
  ...
  day_over_day_change,
  CASE
    WHEN day_over_day_change > 0 THEN 'INCREASE'
    WHEN day_over_day_change < 0 THEN 'DECREASE'
    ELSE 'EQUAL'
  END AS trend
FROM daily_trends
ORDER BY order_date;